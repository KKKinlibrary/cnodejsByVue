  ...
  <div>
    ...
    <el-dialog title="添加分类" :visible.sync="addCateDialogVisible" width="50%" @close="addCateDoalogClosed">
      ...
      >
        ...
        <el-form-item label="父级分类" clearable>
            <!-- options用来指定数据源 -->
            <!-- props用来指定配置对象 -->
          <el-cascader
...
export default {
  ...
  methods: {
    ...
    arraySpanMethod({ row, column, rowIndex, columnIndex }) {
      //当前行row、当前列column
      if (columnIndex === 1 && row.cat_level === 0) {
        column, rowIndex
        console.log('111', '       ', row)
        console.log(row.children, '  sdfs%%%%%%%%%%%% ', row.children.length)
        console.log('222')
        return {
    ...
    async getParentCateList() {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('获取父级分类数据失败555')
      }
    ...
    parentCateChanged(){
        ...
        // 若 this.selectedKeys 长度等于0， 说明没有选中任何选项
