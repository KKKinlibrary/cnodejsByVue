...
export default {
  data() {
    ...
    var checkEmail = (rule, value, cb) => {
      const regEmail = /^([0-9a-zA-Z\-_.])+@[0-9a-zA-Z-_.]+\.([0-9a-zA-Z\-_]{2,4})$/
    ...
    var checkMobile = (rule, value, cb) => {
      const regEmail = /^(0|86|17951)?1[3|4|5|6|7|8|9][0-9]{9}$/
      if (!regEmail.test(value)) {
  ...
  methods: {
    async getUserList() {
      ...
      if (res.meta.status !== 200)
        return this.$message.error('获取用户列表失败QAQ')
    ...
    addUser() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        if (res.meta.status !== 201)
          return this.$message.error('添加用户失败555')
